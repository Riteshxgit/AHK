...
; Configurable delay before starting to repeat the key press (in milliseconds)
HoldDelay := 20  ; Adjust this value to change the delay before repeating starts
; Configurable repeat rate (in milliseconds)
RepeatRate := 1  ; Adjust this value to change the speed of the repeated key presses

; Handle Up arrow key
$Up::
    KeyWait, Up, T%HoldDelay%
    if ErrorLevel
    {
        SetTimer, RepeatUp, %RepeatRate%
    }
    else
    {
        Send, {Up}
...
$Left::
    KeyWait, Left, T%HoldDelay%
    if ErrorLevel
    {
        SetTimer, RepeatLeft, %RepeatRate%
    }
    else
    {
        Send, {Left}
...
Left Up::
    SetTimer, RepeatLeft, Off
Return

; Handle Down arrow key
$Down::
    KeyWait, Down, T%HoldDelay%
    if ErrorLevel
    {
        SetTimer, RepeatDown, %RepeatRate%
    }
    else
    {
        Send, {Down}
...
$Right::
    KeyWait, Right, T%HoldDelay%
    if ErrorLevel
    {
        SetTimer, RepeatRight, %RepeatRate%
    }
    else
    {
        Send, {Right}
